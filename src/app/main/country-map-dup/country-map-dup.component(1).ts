...
export class CountryMapDupComponent {
  ...
  async downloadMapData() {
    ...
    try {
      this.isLoading = true; 
      await this.countryDownloadStatistics.updateanddownloadpdf();
    ...
    filter = (node: HTMLElement) => {
      const exclusionClasses = ['download', 'downloadpdf', 'leaflet-control-zoom', 'leaflet-control-fullscreen', 'leaflet-control-zoomin', 'ResetMap', 'DownloadMaps', 'download-buttons'];
  ...
  async downloadMapImage(downloadpdf : boolean) {
    ...
    try {
        const mapElement = document.getElementById('map-country-ftp') as HTMLElement;
        if (!mapElement) {
   ...
   generatePDF(imageDataUrl: string) {
    ...
    private initMap(): void {
      this.map = L.map('map-country-ftp', {
        center: [24, 81.9629],
    ...
    private toggleLogoPosition(isFullscreen: boolean): void {
      ...
      const resetButton = this.elRef.nativeElement.querySelector('#resetButton-country');
      ...
      const borderRemove = this.elRef.nativeElement.querySelector('#border-remove-country_dup')
      ...
      this.http.get('assets/geojson/INDIA_COUNTRY.json').subscribe((res: any) => {
        const districtLayer = L.geoJSON(res, {
          ...
          onEachFeature: (feature: any, layer: any) => {
            ...
            const dailyrainfall = matchedData && matchedData.actual_rainfall !== null && matchedData.actual_rainfall != undefined && !Number.isNaN(matchedData.actual_rainfall) ? this.constants.trimToOneDecimals(matchedData.actual_rainfall) : 'NA';
            const normalrainfall = matchedData && matchedData.actual_rainfall !== null && !Number.isNaN(matchedData.rainfall_normal_value) ? this.constants.trimToOneDecimals(parseFloat(matchedData.rainfall_normal_value)) : 'NA';
