...
@Component({
  selector: 'app-east-north-east-region-actual',
  templateUrl: './east-north-east-region-actual.component.html',
  styleUrls: ['./east-north-east-region-actual.component.css']
})
export class EastNorthEastRegionActualComponent implements AfterViewInit{
  ...
  districtdatacum: any[] = [];
    ...
    private calculateZoomLevel(width: number, height: number): number {
      const zoomLevel = Math.log2(Math.max(width, height) / 29);
    ...
    onWindowResize() {
      if (!this.isFullscreen()) {
        ...
        if (this.map) {
          ...
          this.map.setView([25, 89.9999], this.initialZoom);
        }
      }
    }
  
    resetMap(): void {
      this.map.setView([25, 89.9999]);
    }
    ...
    private initMap(): void {
      this.map = L.map("map-east-north-east", {
        center: [25, 89.9999],
    ...
    private toggleLogoPosition(isFullscreen: boolean): void {
      ...
      const country_val = this.elRef.nativeElement.querySelector(
        "#country_values_east-north-east"
      );

    const borderRemove = this.elRef.nativeElement.querySelector('#border-remove-east-north-east')
