...
@Component({
  selector: "app-central-india-region-actual",
  templateUrl: "./central-india-region-actual.component.html",
  styleUrls: ["./central-india-region-actual.component.css"],
})
export class CentralIndiaRegionActualComponent implements AfterViewInit {
  districtdatacum: any[] = [];
  ...
  async loadStats() {
    ...
    try {
      await this.downloadPdf$.updateandViewpdfFromDataEntryCustom("1", this.fromDate, this.fromDate);
  ...
  private calculateZoomLevel(width: number, height: number): number {
    const zoomLevel = Math.log2(Math.max(width, height) / 38);
  ...
  onWindowResize() {
    if (!this.isFullscreen()) {
      ...
      if (this.map) {
        ...
        this.map.setView([22, 77.9999], this.initialZoom);
      }
    }
  }

  resetMap(): void {
    this.map.setView([22, 77.9999]);
  }
  ...
  private initMap(): void {
    this.map = L.map("map-central-region", {
      center: [22, 77.9999],
