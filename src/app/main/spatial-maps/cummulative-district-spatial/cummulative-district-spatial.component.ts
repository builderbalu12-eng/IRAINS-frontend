...
@Component({
  selector: 'app-cummulative-district-spatial',
  templateUrl: './cummulative-district-spatial.component.html',
  styleUrls: ['./cummulative-district-spatial.component.css']
})
export class CummulativeDistrictSpatialComponent {
    ...
    private loadGeoJSON(): void {
      ...
      this.http.get("assets/geojson/INDIA_DISTRICT.json").subscribe((res: any) => {
        res.features.forEach((feature: any) => {
          ...
          const intensity = this.getRainfallIntensity(rainfall);
    
          if (intensity > 0) {
            heatPoints.push([centroid.lat, centroid.lng, intensity]);
          }
        });
    ...
    private getRainfallIntensity(rainfall: any): number {
        if (rainfall == null || rainfall === "NA") return 0;

        const val = Math.round(rainfall);

        if (val >= 60) return 1.0;
        if (val >= 20 && val < 60) return 0.8;
        if (val >= -19 && val < 20) return 0.6;
        if (val >= -59 && val < -19) return 0.4;
        if (val >= -99 && val < -59) return 0.2;
