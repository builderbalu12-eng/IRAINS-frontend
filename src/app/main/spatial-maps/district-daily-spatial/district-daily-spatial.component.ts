...
export class DistrictDailySpatialComponent implements OnInit, OnDestroy{
  ...
  private initStationObservationMap(): void {
    ...
    const drawControl = new L.Control.Draw({
      ...
      draw: {
        // polyline: false, // Allow drawing polylines
        // polygon: false,  // Allow drawing polygons
        // rectangle: false,
        // circle: false,
        // marker: false,
      },
    ...
    this.stationObservationMap.on(L.Draw.Event.CREATED, (event: any) => {
      ...
      if (this.isPlottingEnabled) {
        // Store the coordinates of the drawn shape when plotting is enabled
        const coordinates = layer.getLatLngs();
        this.drawnCoordinates.push(coordinates);
        console.log("Coordinates drawn:", this.drawnCoordinates);
      }
  ...
  loadGeoJSON(): void {
    this.http.get("assets/geojson/INDIA_STATE.json").subscribe((res: any) => {
      const heatData: [number, number, number][] = []; // Store [lat, lng, intensity]
  
      // Process GeoJSON and add to the map
      L.geoJSON(res, {
        style: (feature: any) => {
          return {
            weight: 1,
            opacity: 1,
            color: "black",
            fillOpacity: 0.2, // Optional: Adjust transparency for the regions
          };
        },
        onEachFeature: (feature: any, layer: any) => {
          ...
          if (feature.geometry.type === "Polygon") {
            coordinates[0].forEach((coord: number[]) => {
              heatData.push([coord[1], coord[0], 0.5]); // [lat, lng, intensity]
            });
          ...
          // Optional: Add popup information if needed
          layer.bindPopup(`<strong>State:</strong> ${feature.properties.NAME}`);
        },
      }).addTo(this.stationObservationMap);
  
      // Create heatmap layer using the extracted heatData
      const heatLayer = (L as any).heatLayer(heatData, {
        radius: 25, // Adjust radius of the heat points
        blur: 15,   // Adjust blur intensity
        maxZoom: 10, // Maximum zoom level for heatmap
        gradient: {
          0.1: "blue",
          0.5: "lime",
          1.0: "red", // Adjust color gradient
        },
      });
  
      // Add the heatmap layer to the map
      heatLayer.addTo(this.stationObservationMap);
    });
  }
  ...
  onMouseMove(event: MouseEvent): void {
    if (this.isResizing) {
      ...
      this.chartHeight = `${newHeight * 10}px`; // Adjust the factor as needed
