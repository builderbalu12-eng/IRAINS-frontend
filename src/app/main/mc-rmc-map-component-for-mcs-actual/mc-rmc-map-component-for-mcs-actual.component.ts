...
@Component({
  ...
  styleUrls: ['./mc-rmc-map-component-for-mcs-actual.component.css']
})
...
async downloadMapImage(downloadpdf : boolean) {
  ...
  try {
      const mapElement = document.getElementById('map-district-actual') as HTMLElement;
      if (!mapElement) {
 ...
 generatePDF(imageDataUrl: string) {
  ...
  private initMap(): void {
    this.map = L.map('map-district-actual', {
      center: [18, 78.9629],
