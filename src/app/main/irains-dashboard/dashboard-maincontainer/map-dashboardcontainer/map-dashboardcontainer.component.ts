...
//     // Ensure selectedDate is in sync with startDate/endDate from parent
...
export class MapDashboardcontainerComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
  ...
  @Input() maxDate: string = '';
  @Output() layerSelected = new EventEmitter<string>();
  @Output() dateChanged = new EventEmitter<{ startDate: string; endDate: string }>(); // New output for date changes
  ...
  onDateChange() {
    if (this.selectedDate) {
      this.startDate = this.selectedDate;
      this.endDate = this.selectedDate;
      this.dateChanged.emit({ startDate: this.startDate, endDate: this.endDate }); // Emit date change
      this.fetchAllData();
    }
  }
