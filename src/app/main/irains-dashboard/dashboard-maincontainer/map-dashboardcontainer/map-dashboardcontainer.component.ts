...
import { Component, OnInit, OnChanges, SimpleChanges, AfterViewInit, Renderer2, Input, Output, EventEmitter, ViewChild, ElementRef } from '@angular/core';
import { HttpClient } from '@angular/common/http';
import * as L from 'leaflet';
import { StateService } from 'src/app/services/state/state.service';
import { DistrictService } from 'src/app/services/district/district.service';
...
export class MapDashboardcontainerComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  @Input() isActual: boolean = false;
  @Input() maxDate: string = '';
  @Output() layerSelected = new EventEmitter<string>(); // Emit layer selection changes
  ...
  onSelectLayer(layerId: string) {
    ...
    this.updateMap();
    this.layerSelected.emit(layerId); // Emit the selected layer to parent
  }
