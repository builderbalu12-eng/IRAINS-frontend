...
export class MapChartsComponent implements OnInit, OnChanges {
  ...
  @Output() selectedPlaceChange = new EventEmitter<{
    layer: string;
    code: string;
    name: string;
  }>();
  ...
  private fetchPlaces(): void {
    ...
    if (!service) {
      console.error(
        `No service available for layer: ${this.selectedLayer} at`,
        new Date().toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })
      );
      this.availablePlaces = [];
      this.cdr.detectChanges();
    ...
    service.fetchData().subscribe({
      next: (res: any) => {
        ...
        if (
          ...
          (!this.selectedPlace.code ||
            !this.availablePlaces.some(
              (p) => p.code === this.selectedPlace.code
            ))
        ) {
          this.selectedPlace = {
            layer: this.selectedLayer,
      ...
      error: (err: any) => {
        console.error(
          `Error fetching places for ${this.selectedLayer}:`,
          err,
          "at",
          new Date().toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })
        );
  ...
  private getServiceForDropdown(layer: string): any {
    switch (layer) {
      case "region":
        return this.getRegionService;
      case "state":
        return this.getStateService;
      case "subdivision":
        return this.getSubDivisionService;
      case "district":
        return this.getDistrictService;
      case "block":
        return this.getBlockService;
      default:
        return null;
    }
  }
  ...
  private async fetchChartData(): Promise<void> {
    ...
    if (!service || !method) {
      console.error(
        `No service or method available for layer: ${this.selectedPlace.layer}`
      );
    ...
    try {
      ...
      if (!res.success || !res.data) {
        console.warn(
          `No valid data received for ${this.selectedPlace.layer}:`,
          res.message
        );
        this.isChartLoading = false;
        this.isHighestRecordedLoading = false;
      ...
      const uniqueData = Array.from(
        new Map(
          res.data.map((item) => [
            item.date + item[this.getDataCodeKey(this.selectedPlace.layer)],
            item,
          ])
        ).values()
      );
      ...
      this.dates.forEach((date) => {
        const item = this.findItemForPlaceByDate(
          uniqueData,
        ...
        if (item) {
          ...
          console.warn(
            `No chart data found for date ${date} and code ${this.selectedPlace.code} in layer ${this.selectedPlace.layer}`
          );
      ...
      if (!topNService || !topNMethod) {
        ...
        try {
          ...
          if (topNRes.success && topNRes.data) {
            this.highestRecorded = topNRes.data
              .filter((item) => {
                const actual = parseFloat(
                  (item.actual_rainfall as string) ?? "0"
                );
                return !isNaN(actual) && actual < 999 && item.date;
              })
              .sort(
                (a, b) =>
                  parseFloat((b.actual_rainfall as string) ?? "0") -
                  parseFloat((a.actual_rainfall as string) ?? "0")
              )
              .slice(0, 5)
              .map((item) => ({
            ...
            console.log(
              `Filtered highest recorded data for ${this.selectedPlace.layer} (${codeValue}):`,
              this.highestRecorded
            );
            if (this.highestRecorded.length === 0) {
              console.warn(
                `No valid highest recorded data found for ${this.selectedPlace.layer} (${codeValue})`
              );
            }
          } else {
            console.warn(
  ...
  private getTopNMethodName(layer: string): string {
    switch (layer) {
      case "country":
        return "fetchTopNCountries";
      case "region":
        return "fetchTopNRegions";
      case "state":
        return "fetchTopNStates";
      case "subdivision":
        return "fetchTopNSubdivisions";
      case "district":
        return "fetchTopNDistricts";
      case "block":
        return "fetchTopNBlocks";
      default:
        return "";
    }
  }
  ...
  private findItemForPlaceByDate(
    data: RainfallData[],
    layer: string,
    code: string,
    date: string
  ): RainfallData | undefined {
    const codeKey = this.getDataCodeKey(layer);
    console.log(
      `Searching for code ${code} and date ${date} with key ${codeKey} in`,
      data
    );
    return data.find(
      (d) =>
        d.date === date &&
        (layer === "country"
          ? d.country_name?.trim() === code.trim()
          : String(d[codeKey] ?? "").trim() === String(code).trim())
    );
  }
            ...
            dataLabels: {
              ...
              formatter: (function (component) {
                return function (this: any) {
                  const index = this.point.index ?? 0;
                  const departure =
                    component.departureData[index].toFixed(1) + "%";
                  return departure;
                };
              })(this),
              style: {
  ...
  private updateRegions() {
    ...
    const countryItem = {
      ...
      normal: parseFloat(
        (country.rainfall_normal_value as string) ?? "0"
      ).toFixed(1),
      departure:
        parseFloat((country.departure as string) ?? "0").toFixed(1) + "%",
    };
  ...
  private fetchTop5Data() {
    ...
    this.top5Title = `Top 5 ${layerLabel} - ${this.formatDateToDDMMYYYY(
      this.startDate
    )}`;
    ...
    service.fetchData(params).subscribe({
      next: (res) => {
        ...
        data = data
          .filter(
            (d: RainfallData) =>
              d[actualKey] != null && !isNaN(parseFloat(d[actualKey] as string))
          )
          .sort(
            (a: RainfallData, b: RainfallData) =>
              parseFloat(b[actualKey] as string) -
              parseFloat(a[actualKey] as string)
          )
          .slice(0, 5);
