      ...
      if (!topNService || !topNMethod) {
        ...
        try {
          ...
          const topNRes = await lastValueFrom((topNService as any)[topNMethod](params)) as {
            success: boolean;
            message: string;
            data: { topByActual: RainfallData[] };
          };

          // Log full API response
          console.log(`Full API response for ${topNMethod}:`, topNRes);

          // Use only topByActual data
          const topNData = topNRes.data?.topByActual || [];

          // Debugging: Log the raw topByActual data
          ...
          this.highestRecorded = topNData
            .filter(item => {
              ...
              return !isNaN(actual) && actual < 999;
            })
            .slice(0, 5)
            .map(item => ({
