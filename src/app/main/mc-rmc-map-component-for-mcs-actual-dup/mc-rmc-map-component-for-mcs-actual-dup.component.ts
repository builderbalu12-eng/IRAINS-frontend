...
@Component({
  selector: 'app-mc-rmc-map-component-for-mcs-actual-dup',
  templateUrl: './mc-rmc-map-component-for-mcs-actual-dup.component.html',
  styleUrls: ['./mc-rmc-map-component-for-mcs-actual-dup.component.css']
})
export class McRmcMapComponentForMcsActualDupComponent {
  ...
  //   console.log('loggedInUserObject for MC and RMC', this.loggedInUserObject.data[0].mcorhq);
  ...
  private checkUrl() {
    ...
    this.showHeader = this.router.url.includes('allstatemaps');
  }
...
async downloadMapImage(downloadpdf : boolean) {
  ...
  try {
      const mapElement = document.getElementById('map-district-actual-ftp_dup') as HTMLElement;
      if (!mapElement) {
 ...
 generatePDF(imageDataUrl: string) {
  ...
  private calculateInitialZoom(customZoomfactor : any = undefined): void {
    ...
    this.initialZoom = this.calculateZoomLevel(cardWidth, cardHeight, customZoomfactor);
  }
  
  private calculateZoomLevel(width: number, height: number, customZoomfactor : any = undefined): number {
    console.log('zoooom', (customZoomfactor === undefined ? 20: customZoomfactor))
    const zoomLevel = Math.log2(Math.max(width, height) / (customZoomfactor === undefined ? 20: customZoomfactor)); 
    return zoomLevel;
  }
  ...
  onWindowResizer() {
    if (!this.isFullscreen()) {
      ...
      this.calculateInitialZoom(
        this.mcRMCService.getZoomFactor(this.selectedMcName)
      );
  ...
  onWindowResize() {
    if(!this.isFullscreen()){
      ...
      this.calculateInitialZoom(this.mcRMCService.getZoomFactor(this.selectedMcName))
  ...
  resetMap(): void {
    this.map.setView(this.mcRMCService.getCordinates(this.selectedMcName), this.initialZoom+1);
  }
  ...
  selectedMcName : any = 'MC Hyderabad';

  onSelectChange(event: Event): void {
    console.log(this.listOfmsRMCs)
    const target = event.target as HTMLSelectElement;
    this.selectedMcName = target.value;
    this.selectedUrl = this.mcRMCService.getMcRMCsJson()[this.selectedMcName].url || null;
    if (this.selectedUrl) {
  ...
  private initMap(): void {
    this.map = L.map('map-district-actual-ftp_dup', {
      center: [18, 78.9629],
  ...
  private loadGeoJSON(httpUrl : any): void {
    ...
    this.http.get(httpUrl).subscribe((res: any) => {
      ...
      this.map.setView(this.mcRMCService.getCordinates(this.selectedMcName));
