...
@Component({
  ...
  styleUrls: ['./state-rainfall-map-cummulative.component.css']
})
export class StateRainfallMapCummulativeComponent {
    ...
    async downloadMapData() {
      ...
      try {
        ...
        if(this.selectedMode.selectedMode == 'Unified'){
          await this.downloadStatistcs.updateanddownloadpdfCustom(this.StartDate, this.EndDate);
        }else{
          await this.downloadStatistcs.updateanddownloadpdfFromDataEntryCustom(this.StartDate, this.EndDate);
