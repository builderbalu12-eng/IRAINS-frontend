...
@Component({
  selector: 'app-district-east-and-north-east-rainfall-map-cummulative',
  templateUrl: './district-east-and-north-east-rainfall-map-cummulative.component.html',
  styleUrls: ['./district-east-and-north-east-rainfall-map-cummulative.component.css']
})
export class DistrictEastAndNorthEastRainfallMapCummulativeComponent {
      ...
      // never actually assigned, see fetchBackend()'s commented-out
      // assignment), rendered by the shared <app-rainfall-stats-panel>.
      // No category-breakdown sub-table (buildCategoryStats() N/A for
      // district data). To revert to a map-only page: delete these
      // fields, loadStats() below, and the two `this.loadStats();` call
      // sites (search this file for "this.loadStats()") — plus the
    ...
    formatDateForDisplay(date: Date): string {
      ...
      async loadStats() {
        ...
        try {
          await this.downloadPdf$.updateandViewpdfFromDataEntryCustom("2", this.fromDate, this.toDate);
      ...
      private calculateZoomLevel(width: number, height: number): number {
        const zoomLevel = Math.log2(Math.max(width, height) / 35);
      ...
      private loadGeoJSON(): void {
        this.http
          .get("assets/geojson/regions/EAST_AND_NORTH_EAST_INDIA.json")
          .subscribe((res: any) => {
