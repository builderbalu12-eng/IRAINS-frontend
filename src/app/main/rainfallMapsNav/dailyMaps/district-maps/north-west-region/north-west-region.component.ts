...
@Component({
  selector: 'app-north-west-region',
  templateUrl: './north-west-region.component.html',
  styleUrls: ['./north-west-region.component.css']
})
export class NorthWestRegionComponent implements AfterViewInit{
    ...
    districtdatacum: any[] = [];
      ...
      async downloadMapData() {
        ...
        try {
          ...
          if(this.selectedMode.selectedMode == 'Unified'){
            ...
            await this.downloadPdf$.updateanddownloadpdfCustom("3", this.fromDate, this.fromDate);
  
          }else{
            await this.downloadPdf$.updateanddownloadpdfFromDataEntryCustom("3", this.fromDate, this.fromDate);
          }        
      ...
      async loadStats() {
        ...
        try {
          await this.downloadPdf$.updateandViewpdfFromDataEntryCustom("3", this.fromDate, this.fromDate);
      ...
      private calculateZoomLevel(width: number, height: number): number {
        const zoomLevel = Math.log2(Math.max(width, height) / 33);
      ...
      onWindowResize() {
        if (!this.isFullscreen()) {
          ...
          if (this.map) {
            ...
            this.map.setView([29.444, 76.9999], this.initialZoom);
          }
        }
      }
    
      resetMap(): void {
        this.map.setView([29.444, 76.9999]);
      }
      ...
      private initMap(): void {
        this.map = L.map("map-north-west", {
          center: [29.444, 76.9999],
      ...
      private toggleLogoPosition(isFullscreen: boolean): void {
        ...
        const country_val = this.elRef.nativeElement.querySelector(
          "#country_values_north-west"
        );
  
      const borderRemove = this.elRef.nativeElement.querySelector('#border-remove-north-west')
        ...
        if (isFullscreen) {
          ...
          this.renderer.setStyle(directionCompass, "right", "30%");
    ...
    private loadGeoJSON(): void {
      this.http
        .get("assets/geojson/regions/NORTH_WEST_INDIA.json")
        .subscribe((res: any) => {
