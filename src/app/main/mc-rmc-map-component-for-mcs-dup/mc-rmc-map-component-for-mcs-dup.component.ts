...
@Component({
  selector: 'app-mc-rmc-map-component-for-mcs-dup',
  templateUrl: './mc-rmc-map-component-for-mcs-dup.component.html',
  styleUrls: ['./mc-rmc-map-component-for-mcs-dup.component.css']
})

export class McRmcMapComponentForMCsDupComponent {
...
async downloadMapImage(downloadpdf : boolean) {
  ...
  try {
      const mapElement = document.getElementById('map-district-ftp_dup') as HTMLElement;
      if (!mapElement) {
 ...
 generatePDF(imageDataUrl: string) {
  ...
  private initMap(): void {
    this.map = L.map('map-district-ftp_dup', {
      center: [18, 78.9629],
