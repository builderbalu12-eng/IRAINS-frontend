...
@Component({
  selector: 'app-district-actual-map-dup',
  templateUrl: './district-actual-map-dup.component.html',
  styleUrls: ['./district-actual-map-dup.component.css']
})
export class DistrictActualMapDupComponent implements AfterViewInit{
  ...
  private initMap(): void {
    this.map = L.map("map-district-actual-ftp", {
      center: [24, 81.9629],
      ...
      .subscribe((res: any) => {
        const districtLayer = L.geoJSON(res, {
          style: (feature: any) => {
            ...
            const dailyrainfall =
            matchedData &&
            matchedData.actualrainfall !== null &&
            matchedData.actualrainfall != undefined &&
            !Number.isNaN(matchedData.actualrainfall)
              ? this.constants.trimToOneDecimals(
                  matchedData.actualrainfall
                )
              : "NA";
           // console.log('dailyrainfall actual dups', dailyrainfall);
          ...
          onEachFeature: (feature: any, layer: any) => {
            ...
            const actualrainfall =
              matchedData && !Number.isNaN(matchedData.actual_rainfall)
                ? this.constants.trimToOneDecimals(
                    parseFloat(matchedData.actual_rainfall)
                  ) + " mm"
                : "NA";
            const popupContent = `
 ...
 <div style="color: #002467; font-weight: bold; font-size: 13px;">NORMAL RAINFALL: ${actualrainfall}</div>
 </div>
 `;
  ...
  //   const numericId = this.constants.trimToZeroDecimals(rainfall);
