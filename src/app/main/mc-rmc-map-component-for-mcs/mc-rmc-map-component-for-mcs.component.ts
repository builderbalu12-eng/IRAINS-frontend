...
@Component({
  ...
  styleUrls: ['./mc-rmc-map-component-for-mcs.component.css']
})
export class McRmcMapComponentForMCsComponent {
  ...
  constructor(
    ...
    const initWithEffectiveDate = (effectiveDate: Date) => {
      ...
      this.dataService.fromAndToDate$.subscribe((value) => {
        ...
        // Guarded because selectedMcName is only set in ngOnInit, and
        // getCordinates() throws on an unknown name.
...
async downloadMapImage(downloadpdf : boolean) {
  ...
  try {
      const mapElement = document.getElementById('map-district-ftp_') as HTMLElement;
      if (!mapElement) {
 ...
 generatePDF(imageDataUrl: string) {
  ...
  // url : any = this.mcRMCService.getMcRMCsJson()[this.loggedInUserObject.data[0].name].url
  // this.loadGeoJSON(url);
  selectedMcName : any = '';
  ...
  private initMap(): void {
    this.map = L.map('map-district-ftp_', {
      center: [18, 78.9629],
