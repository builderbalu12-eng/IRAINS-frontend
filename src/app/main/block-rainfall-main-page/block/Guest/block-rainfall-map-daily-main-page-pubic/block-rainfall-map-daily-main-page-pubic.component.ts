...
@Component({
  selector: 'app-block-rainfall-map-daily-main-page-pubic',
  templateUrl: './block-rainfall-map-daily-main-page-pubic.component.html',
  styleUrls: ['./block-rainfall-map-daily-main-page-pubic.component.css']
})
export class BlockRainfallMapDailyMainPagePubicComponent {
  ...
  onStateChange(event: any): void {
    ...
    if (this.selectedState.length > 0) {
      ...
      const filteredDistricts = this.districts[0]?.data.filter(
        (district: any) =>
          this.selectedState.some(
            (state: any) => state.state_code === district.state_code
          )
      );
      this.filterDistrict = filteredDistricts || [];
      console.log("Filtered Districts by States:", this.filterDistrict);
  ...
  private updateMap(): void {
    ...
    const filteredGeoJson = this.geoJsonData.features.filter((feature: any) => {
      ...
      if (!props || !geom) {
        console.warn(
          "Invalid feature, missing properties or geometry:",
          feature
        );
        return false;
      }
      ...
      if (this.selectedMC?.length > 0) {
        const selectedMCs = this.selectedMC.map(
          (r: any) => r.centre_type + " " + r.centre_name
        );
      ...
      if (this.selectedRMC?.length > 0) {
        const selectedRMCs = this.selectedRMC.map(
          (r: any) => r.centre_type + " " + r.centre_name
        );
      ...
      if (this.selectedState?.length > 0) {
        const selectedStates = this.selectedState.map((st: any) =>
          st.state_code.toString()
        );
      ...
      if (this.selectedDistrictData?.length > 0) {
        const selectedDistricts = this.selectedDistrictData.map(
          (d: any) => d.district_code
        );
      ...
      if (this.selectedBlockData?.length > 0) {
        const selectedBlocks = this.selectedBlockData.map(
          (b: any) => b.block_code
        );
