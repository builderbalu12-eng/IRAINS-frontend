...
export class CountryMapComponent {
  ...
  async downloadMapData() {
    ...
    try {
      this.isLoading = true; 
      await this.countryDownloadStatistics.updateanddownloadpdfFromDataEntry();
  ...
  filter = (node: HTMLElement) => {
    const exclusionClasses = ['download', 'downloadpdf', 'leaflet-control-zoom', 'leaflet-control-fullscreen', 'leaflet-control-zoomin', 'DownloadMaps', 'ResetMap',  'download-buttons'];
...
async downloadMapImage(downloadpdf : boolean) {
 ...
 generatePDF(imageDataUrl: string) {
  ...
  private toggleLogoPosition(isFullscreen: boolean): void {
    ...
    const borderRemove = this.elRef.nativeElement.querySelector('#border-remove-country')
