...
import { MCRMCsServiceSubdiv } from "src/app/services/MC-RMCs/mcRmcSubdiv.service";
...
@Component({
  ...
  templateUrl: './rainfallmap-subdiv-mc-rmc.component.html',
  styleUrls: ['./rainfallmap-subdiv-mc-rmc.component.css']
})
export class RainfallmapSubdivMcRmcComponent {
    ...
    allSubdivsLocatedinCurrentMc: any = [];
    selectedSubdiv: any = '';
  subdivisions: any;
isLoadingSubdivs: any;
  selectedMode: any;
    ...
    async downloadMapData() {
      ...
      try {
        ...
        if(this.selectedMode.selectedMode=='Unified'){
          await this.downloadPdf$.updateanddownloadpdfCustom(this.allSubDivCodesinSelectedSet, this.fromDate, this.toDate);
        }else{
        await this.downloadPdf$.updateanddownloadpdfFromDataEntryCustom(this.allSubDivCodesinSelectedSet, this.fromDate, this.toDate);
    ...
    private checkUrl() {
      this.showHeader = this.router.url.includes("subdiv-map-mc-rmc");
    }
  
    resetMapSmallScreen(): void {
  
      const zoomFactor = this.mcRMCService.getZoomFactor(this.selectedMcName, this.selectedSubdiv);
      this.map.setView(
        this.mcRMCService.getCoordinates(this.selectedMcName, this.selectedSubdiv),
        zoomFactor
      );
    }
    ...
    async fetchBackend() {
      ...
      if(this.selectedMode.selectedMode == 'Unified'){
        this.district.fetchDataFtp(data).subscribe((res) => {
          ...
          this.subdivisions = this.mcRMCService.getMcRMCsJson()[this.loggedInUserObject.data[0].name].subdivisions;
        
          this.allSubdivsLocatedinCurrentMc = this.subdivisions.map((state: any) => {
            return {
          ...
          if (this.allSubdivsLocatedinCurrentMc.length > 0) {
            this.selectedSubdiv = this.allSubdivsLocatedinCurrentMc[0].name; // Set first subdivision url
            console.log(this.allSubdivsLocatedinCurrentMc[0].name)
            this.loadGeoJSON(this.allSubdivsLocatedinCurrentMc[0].url);    // Load first subdivision map
          }
          console.log('consoling allSubdivsLocatedinCurrentMc', this.allSubdivsLocatedinCurrentMc);
        
          this.StartDate = this.convertToIndianDateFormat(this.StartDate);
          this.EndDate = this.convertToIndianDateFormat(this.EndDate);
  
        this.isLoadingSubdivs = false
    ...
    async downloadMapImage(downloadpdf: boolean) {
      ...
      try {
        ...
        if (!this.isFullscreen()) {
          ...
          link.download = `SUBDIVISION ${this.selectedSubdiv} DISTRICT_RAINFALL_MAP_COUNTRY_INDIA_cd.jpeg`;
          link.href = dataUrl;
    ...
    onWindowResizer() {
      if (!this.isFullscreen()) {
        ...
        console.log("selected state name", this.selectedMcName, this.selectedSubdiv, 'hi');
        this.calculateInitialZoom(
          this.mcRMCService.getZoomFactor(this.selectedMcName, this.selectedSubdiv)
        );
        // if (this.map) {
    ...
    private initMap(): void {
      ...
      this.map = L.map("map-MC-RMC-subdiv-ftp_dup", {
    ...
    onSubdivChange(): void {
      const selectedSubdivData = this.subdivisions.find((state: any) => state.name === this.selectedSubdiv);
      if (selectedSubdivData) {
        this.loadGeoJSON(selectedSubdivData.url);
      } else {
        console.error("No state data found for name:", this.selectedSubdiv);
      }
    }
      ...
      this.http.get(httpUrl).subscribe((geoJsonData: any) => {
        const layer = L.geoJSON(geoJsonData, {
          style: (feature: any) => {
            ...
            this.allSubDivCodesinSelectedSet.add(feature.properties['subdivis_1'])
