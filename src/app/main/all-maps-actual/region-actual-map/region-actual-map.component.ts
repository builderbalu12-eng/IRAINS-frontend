...
export class RegionActualMapComponent {
  ...
  async downloadMapData() {
    ...
    try {
      this.isLoading = true;
      await this.regionStatisticsDownload.updateanddownloadpdfFromDataEntry();
  ...
  private initMap(): void {
    this.map = L.map("map-actual-region", {
      center: [24, 81.9629],
